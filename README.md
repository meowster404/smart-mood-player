...
The Smart Music Player uses advanced natural language processing and Spotify integration to understand user requests and provide personalized music recommendations.
...
│                          User Input                                     │
├─────────────────────────────────────────────────────────────────────────┤
│  • Text Input (typing in chat interface)                                │
...
│                                                                         │
│  • Keyword Analysis:                                                    │
│    - Mood keywords: happy, sad, energetic, calm, romantic, angry        │
│    - Activity keywords: studying, workout, party, work, gaming          │
│    - Genre keywords: rock, pop, jazz, classical, electronic, hip-hop    │
│                                                                         │
│  • Context Awareness:                                                   │
│    - Conversation history tracking                                      │
│    - Previous intent memory                                            │
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌─ SongSearch ─┐    ┌─ ArtistSearch ─┐    ┌─ ActivitySearch ─┐         │
│  │              │    │                │    │                  │         │
│  │ • Parse song │    │ • Extract      │    │ • Match activity │         │
│  │   title and  │    │   artist name  │    │   keywords       │         │
│  │   artist     │    │ • Get top      │    │ • Map to mood    │         │
│  │ • Search     │    │   tracks       │    │ • Find matching  │         │
│  │   Spotify    │    │ • Display      │    │   playlists      │         │
│  │   tracks     │    │   results      │    │ • Show results   │         │
│  └──────────────┘    └────────────────┘    └──────────────────┘         │
│           │                    │                     │                   │
│           ▼                    ▼                     ▼                   │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐         │
│  │   Spotify API   │  │   Spotify API   │  │   Spotify API   │         │
│  │   Search        │  │   Artist Top    │  │   Playlist      │         │
│  │                 │  │   Tracks        │  │   Search        │         │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘         │
│           │                    │                     │                   │
│           ▼                    ▼                     ▼                   │
...
│  • Interactive Elements: "Open in Spotify" buttons                      │
│  • Visual Enhancements: loading animations, hover effects               │
...
│  • Log Storage in analysis_logs/ directory                             │
└─────────────────────────────────────────────────────────────────────────┘
```
...
1. **Input Reception** 🎤
   - User types message or speaks into microphone
   - Voice input converted to text using SpeechRecognition
   - Input normalized and cleaned

2. **Intent Classification** 🔍
   - Advanced pattern matching using regex
   - Keyword analysis for mood/activity detection
   - Context-aware classification based on conversation history

3. **Query Processing** ⚡
   - **Song Search**: Parse "song by artist" patterns → Spotify track search
   - **Artist Search**: Extract artist name → Get top tracks from Spotify
   - **Activity Search**: Map activity to mood → Find relevant playlists
   - **Chat Response**: Pattern matching against conversation database

4. **Spotify Integration** 🎵
   - OAuth 2.0 authentication via Spotipy library
   - RESTful API calls to Spotify Web API
   - Rate limiting and error handling
   - Result parsing and formatting

5. **Response Generation** 💬
   - Contextual chat responses using trained chatbot model
   - Music results formatted for UI display
   - Interactive elements with Spotify web player integration

6. **Analytics & Learning** 📊
   - Real-time performance metrics collection
   - User satisfaction tracking
   - Response time monitoring
   - Accuracy analysis for continuous improvement

### Technical Architecture

The system is built with a modular architecture:

- **Frontend**: CustomTkinter-based GUI with modern design
- **Backend Processing**: Multi-threaded request handling
- **AI Components**: Pattern matching, keyword analysis, context awareness
- **External APIs**: Spotify Web API integration
- **Data Storage**: JSON configurations, pickle models, log files
- **Analytics**: Performance tracking and visualization

This architecture ensures responsive user experience while handling complex natural language processing and API interactions efficiently.
